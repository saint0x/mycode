...
// Variable substitution function, supports {{var}} format
...
// Default theme configuration - using Nerd Fonts icons and attractive colors
...
// Check if simple theme should be used (fallback)
// When USE_SIMPLE_ICONS env var is set, or when terminal may not support Nerd Fonts
function shouldUseSimpleTheme(): boolean {
  ...
  // Check terminal type (some terminals that don't support complex icons)
...
function canDisplayNerdFonts(): boolean {
  ...
  // Check some common env vars that indicate Nerd Fonts support
  ...
  // By default, assume Nerd Fonts can be displayed (but allow user override via env var)
...
export async function parseStatusLineData(input: StatusLineInput): Promise<string> {
  try {
    ...
    // Get home directory theme config, if none use determined default config
    ...
    // If still no model name, use display_name from input JSON data's model field
    if (!model) {
...
async function renderDefaultStyle(
  ...
  // Traverse module array, render each module
...
function segment(text: string, textFg: string, bgColor: string, nextBgColor: string | null): string {
  ...
  if (nextBgColor != null) {
    ...
    if (nextBgRgb) {
      // Separator: foreground is current segment's background, background is next segment's background
...
async function renderPowerlineStyle(
  ...
  for (let i = 0; i < Math.min(modules.length, 5); i++) {
    ...
    // Generate segment, supporting hex colors
